...
| 13:00–14:00 | **3b. Traces (cobertura parcial)** | Jaeger: trace do `POST /api/v1/biblioteca` no `catalog-api`, com os spans do **outbox** e da publicação do `OrderPlacedEvent`. **Declarar a limitação:** o `payments-api` ainda não está instrumentado, então a cadeia não fecha entre serviços — está registrado em payments-api#20 |
